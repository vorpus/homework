...
class MetaCorgiPerkPackage
  ...
  #   info_method_name = "get_#{method_name.to_s}_info"
  #   happiness_method_name = "get_#{method_name.to_s}_happiness"
  #   info = @shopping_list.send("#{info_method_name}", @package_id)
  #   happiness = @shopping_list.send("#{happiness_method_name}", @package_id)
  #
  #   output_format = method_name.to_s.split("_").map(&:capitalize).join(" ")
  #
  #   result = "#{output_format}: #{info}: #{happiness} licks"
  #   happiness > 30 ? "* #{result}" : result
  #
  # end
  ...
  def self.define_perk(name)
    define_method(name) do
        info_name = "get_#{name.to_s}_info"
        happiness_name = "get_#{name.to_s}_happiness"
        info = @shopping_list.send("#{info_name}", @package_id)
        happiness = @shopping_list.send("#{happiness_name}", @package_id)

        output_format = name.to_s.split("_").map(&:capitalize).join(" ")
